...
def compute_sparse_similarity(query_vec: np.ndarray, doc_vec: np.ndarray) -> float:
    """
    Compute dot product similarity between sparse vectors (dense but sparse arrays).
    
    Args:
        query_vec: Query embedding array
        doc_vec: Document embedding array
...
def compute_metrics(
    ...
    Args:
        query_vecs: Query embedding vectors
        doc_vecs: Document embedding vectors
